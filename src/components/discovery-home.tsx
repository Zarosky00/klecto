        ...
        <footer>
          ...
          {(displayDescription || displayTags.length > 0) && <section className={styles.mediaDetails} aria-label="Image details">
            <button type="button" className={styles.mediaDetailsToggle} onClick={() => setDetailsOpen((open) => !open)} aria-expanded={detailsOpen}>
              <span><small>ABOUT THIS {activeTarget.targetKind === "subcollection" ? "SUBCOLLECTION" : "OBJECT"}</small><strong>{displayDescription ? (detailsOpen || displayDescription.length <= 120 ? displayDescription : `${displayDescription.slice(0, 120).trimEnd()}…`) : "View the details and tags"}</strong></span>
